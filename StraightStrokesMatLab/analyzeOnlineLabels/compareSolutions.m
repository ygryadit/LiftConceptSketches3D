function [agreement, solution_labeling, solution_our] = compareSolutions(solution_labeling, agreement, inds_new, intersections_rec, ours_active, intersections_coordinates2D_act, folder_save, strokes)

 
inds = find(ours_active);


solution_labeling_ = solution_labeling(inds);
agreement_ = agreement(inds);
solution_our_ = ours_active(inds);
...
%  agreement = agreement(mask_likely);
 ...
 intersections_coordinates = [cat(1,intersections_rec(inds_new).coordinates2D);intersections_coordinates2D_act] ;
 %%
 
 solution_labeling  = [solution_labeling, solution_labeling_];
 solution_our  = [solution_our; solution_our_];
 agreement = [agreement, agreement_];
