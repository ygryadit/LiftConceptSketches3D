function [length_commom, length_s1, length_s2, s_new_points2D,dist] = findEndpointsCommonRegionPolyPolyLines(s1_points2D, s2_points2D, r)
...
if d == 1
    [y1,x1,ind,dist,er] = fitPolyLine(x,y);
    
    if er > r*2
        [x_,y_,ind_,dists_,er_] = fitPolyLine(y,x);
        if er_ < er
            y = y_;
            x = x_;
            ind = ind_;
    ...
    [x1,y1,ind,dist,er] = fitPolyLine(y,x);   
    if er > r*2
        [y_,x_,ind_,dists_,er_] = fitPolyLine(x,y);
        if er_ < er
            y = y_;
            x = x_;
            ind = ind_;
...
% dist = dist([ind1(1) ind1(end) ind2(1) ind2(end)]);
...
function [y,x,ind,dist,er] = fitPolyLine(x,y)
[x, ind] = sort(x);
y = y(ind);
% plot(x,y, 'm');
[f, dof] = fit(x, y, 'poly1');
