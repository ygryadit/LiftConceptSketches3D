function objectChangeCamDistAndSaveSVGFrames(strokes_topology, cam_param, folder_name)
...
values(1,:) = [(c(1)-vdir(1)*num_views*step):vdir(1)*step:(c(1)+vdir(1)*num_views*step)];
values(2,:) = [(c(2)-vdir(2)*num_views*step):vdir(2)*step:(c(2)+vdir(2)*num_views*step)];
values(3,:) = [(c(3)-vdir(3)*num_views*step):vdir(3)*step:(c(3)+vdir(3)*num_views*step)];

for i = 1:size(values,2)
    c = values(:,i);
    changeFocalLengthAndSaveSVG(strokes_topology, cam_param, c, i-1, folder_name);    
end

end


function changeFocalLengthAndSaveSVG(strokes_topology, cam_param, c, frame_num, folder_name)
global folder_save;
