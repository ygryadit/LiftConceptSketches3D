function save_as_svg_clusters(strokes_topology, filename)
...
num_clusters = sum(cat(1,strokes_topology(:).primitive_type) == 0);
colors= uint8(colormap(hsv(num_clusters))*255);
ic = 0;
for i = 1:length(strokes_topology)
