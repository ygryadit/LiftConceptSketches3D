% function save_as_svg_confidence_stroke(strokes_topology, filename)
% 
% Description:
%   Saves teh reconstrcution into an svg file, where color coding
%   represents the confidence of
function save_as_svg_confidence_stroke(strokes_topology, folder_save_svg, filename)
...
num_strokes = sum(cat(1,strokes_topology(:).primitive_type) == 0);
num_colors = 100;
colors = uint8(colormap(parula(num_colors))*255);
...
%    end
   ...
   val = strokes_topology(ic).score*(num_colors-1)+1;
   
   step = colors(ceil(val),:) - colors(floor(val),:);

   coef = val - floor(val);
   try
        color = uint8(colors(floor(val),:) + coef*step);
   catch e
       disp(strokes_topology(ic).score);
       rethrow(e);
   end
