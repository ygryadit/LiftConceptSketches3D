function save_as_svg_curved_strokes_intersections_all_v2(strokes, intersections, folder_save, sketch_height, name)
...
for i = 1:length(strokes)   
   ...
   for j = 2:length(strokes(i).points2D)
      str = sprintf('\t \t L %.5f %.5f  \n',strokes(i).points2D(j).x, strokes(i).points2D(j).y);   
      fwrite(fid, str);
   end
   ...
   if strokes(i).primitive_type == 0
       color = [0 0 0];
   else
       color = [51, 204, 51];
   end
   str = sprintf('\n \t \t fill="none" stroke="rgb(%d, %d, %d)" stroke-width="%.5f" stroke-opacity="%.3f"  /> \n',...
                color(1),...
                color(2),...
                color(3),...
...
for i = 1:length(intersections.collinear)
    
    if intersections.collinear(i)
        color = [255, 51, 153]; 
    elseif intersections.tangent(i)
        color = [255, 51, 153]; 
    else
        color = [33, 150, 243]; 
    end
