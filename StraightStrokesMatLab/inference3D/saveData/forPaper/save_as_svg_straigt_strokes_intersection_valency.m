function save_as_svg_straigt_strokes_intersection_valency(strokes, ...
                intersections, int_ind, ...
                pairsInterInter,...
                folder_save, sketch_height, name)
...
%% Lines grouped with the intersection:
...
colors = uint8(hsv(length(ind_lines_intersect))*255);
kc = 1;
...
for i = 1:length(strokes)   
   ...
   if ismember(i, ind_lines_intersect)
       color = colors(kc,:);
       kc = kc + 1;
       opacity = 1.0;
   else
       color = [0 0 0];
       opacity = avg_pressure*0.5;
   end
   str = sprintf(' fill="none" stroke="rgb(%d, %d, %d)" stroke-width="%.25f" stroke-opacity="%.3f" />',...
                color(1),...
                color(2),...
                color(3),...
...
for i = ind_paired_intersections'
