function [sketch, img, scale, intersections] = scaleCenterSketchIntersections(sketch, intersections, img)
...
[sketch, intersections] = scaleCenterVector(sketch, intersections, scale,  dx, dy, min_x,  min_y);
...
function [sketch, intersections] = scaleCenterVector(sketch, intersections, scale, off_x, off_y, min_x, min_y)
...
%         plot([sketch.strokes(i).points(:).x],[[sketch.strokes(i).points(:).y]]);
    ...
    for i = 1:size(intersections.coordinates2D,1)
        intersections.coordinates2D(i,1) = (intersections.coordinates2D(i,1) - min_x)*scale + off_y;
        intersections.coordinates2D(i,2) = (intersections.coordinates2D(i,2) - min_y)*scale + off_x;
    end
end
