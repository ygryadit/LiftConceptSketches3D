...
def generate_bezier_without_tangents(points):

    t = chordLengthParameterize(points)
    p0 = points[0]
    p3 = points[-1]
    ...
    for i in range(1, len(points)-1):
        ...
        a2 = a2 + b2**2
        a12 = a12 + b1*b2
        c1 = c1 + b1*(points[i] - b0*p0 - b3*p3)
        c2 = c2 + b2*(points[i] - b0*p0 - b3*p3)
...
def generateBezier(points, parameters, leftTangent, rightTangent):
    ...
    alpha_l = 0.0 if isclose(det_C0_C1, 0) else det_X_C1 / det_C0_C1
    alpha_r = 0.0 if isclose(det_C0_C1, 0) else det_C0_X / det_C0_C1
...
def computeMaxError(points, bez, parameters):
    ...
    for i, (point, u) in enumerate(zip(points, parameters)):
        dist = sqrt(linalg.norm(bezier.q(bez, u)-point)**2)
        if dist > maxDist:
            maxDist = dist
            splitPoint = i
